...
pub trait AgentContext {
    fn try_output_impl(&self, ch: String, data: AgentData) -> Result<()>;

    fn try_output<S: Into<String>>(&self, ch: S, data: AgentData) -> Result<()> {
        self.try_output_impl(ch.into(), data)
    }

    fn emit_display_impl(&self, key: String, data: AgentData) -> Result<()>;

    fn emit_display<S: Into<String>>(&self, key: S, data: AgentData) -> Result<()> {
        self.emit_display_impl(key.into(), data)
    }

    fn emit_error_impl(&self, message: String) -> Result<()>;

    #[allow(unused)]
    fn emit_error<S: Into<String>>(&self, message: S) -> Result<()> {
        self.emit_error_impl(message.into())
    }
}
